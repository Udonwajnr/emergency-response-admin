...
export const firstAidService = {
  ...
  getGuides: async (params = {}) => {
    try {
      ...
      const response = await api.get(API_ENDPOINTS.FIRST_AID.LIST, { params: queryParams })
      return {
        success: true,
        data: response.data || [],
  ...
  updateGuide: async (guideId, guideData) => {
    try {
      ...
      const response = await api.put(API_ENDPOINTS.FIRST_AID.UPDATE(guideId), updateData)
      return {
        success: true,
        data: response.data,
  ...
  getRecentGuides: async (limit = 10) => {
    try {
      ...
      if (result.success) {
        ...
        const sortedGuides = result.data.sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt)).slice(0, limit)
