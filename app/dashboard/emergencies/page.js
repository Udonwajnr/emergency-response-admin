...
import { AlertTriangle, CheckCircle, Clock, Eye, Loader2, MapPin, Trash2, XCircle } from "lucide-react"
...
export default function EmergenciesPage() {
  ...
  const loadEmergencies = async () => {
    try {
      ...
      if (result.success) {
        ...
        const userIds = [...new Set(result.data.map((emergency) => emergency.userId))]
        const userMap = {}
  ...
  const handleResolveEmergency = async (emergencyId) => {
    try {
      ...
      if (result.success) {
        setEmergencies((prev) =>
          prev.map((emergency) =>
            emergency._id === emergencyId ? { ...emergency, status: "resolved", resolvedAt: new Date() } : emergency,
          ),
        )
  ...
  const handleDeleteEmergency = async () => {
    ...
    try {
      setSubmitting(true)
      const result = await emergencyService.deleteEmergency(emergencyToDelete._id)

      if (result.success) {
        setEmergencies((prev) => prev.filter((emergency) => emergency._id !== emergencyToDelete._id))
  ...
  const filteredEmergencies = emergencies.filter((emergency) => {
    ...
    // Filter by type
    const matchesType = selectedType === "all" || emergency.emergencyType === selectedType

    return matchesSearch && matchesStatus && matchesType
  })

  // Get unique emergency types for filter
  const emergencyTypes = [...new Set(emergencies.map((emergency) => emergency.emergencyType))].filter(Boolean)
  ...
  return (
    <PullToRefresh onRefresh={loadEmergencies}>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Emergency Management</h1>
          <p className="text-muted-foreground text-sm md:text-base">Monitor and manage emergency incidents</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{emergencies.filter((e) => e.status === "active").length}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{emergencies.filter((e) => e.status === "accepted").length}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{emergencies.filter((e) => e.status === "resolved").length}</div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="space-y-4">
          ...
          <MobileSearch onSearch={setSearchTerm} placeholder="Search emergencies..." filters={filterComponent} />
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>List of all emergency incidents</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                    ...
                    <TableRow key={emergency._id}>
                      <TableCell>
                        <div className="flex items-center space-x-3">
                          <Avatar>
                            <AvatarImage src={users[emergency.userId]?.profilePhoto || "/placeholder.svg"} />
                            <AvatarFallback>
                          ...
                          <div>
                            <div className="font-medium">{users[emergency.userId]?.name || "Unknown User"}</div>
                      ...
                      <TableCell>
                        <div className="font-medium">{emergency.emergencyType || "Unknown"}</div>
                        <div className="text-sm text-muted-foreground">
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 mb-1">No emergencies found</h3>
        ...
        <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
          <DialogContent className="max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>Detailed information about the emergency incident</DialogDescription>
            </DialogHeader>
            {selectedEmergency && (
              <div className="space-y-6">
                {/* Status Badge */}
                <div className="flex justify-between items-center">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-lg font-medium">
                      {selectedEmergency.emergencyType || "Unknown Emergency Type"}
                    </span>
                  </div>
                ...
                <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                  <Avatar className="h-16 w-16">
                    <AvatarImage src={users[selectedEmergency.userId]?.profilePhoto || "/placeholder.svg"} alt="User" />
                    <AvatarFallback>
                  ...
                  <div>
                    <h3 className="text-lg font-semibold">{users[selectedEmergency.userId]?.name || "Unknown User"}</h3>
                    <p className="text-muted-foreground">{users[selectedEmergency.userId]?.phone || "No phone"}</p>
                    <p className="text-muted-foreground">{users[selectedEmergency.userId]?.email || "No email"}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-sm font-medium">Emergency Type</Label>
                    <p className="text-sm text-muted-foreground">{selectedEmergency.emergencyType || "Unknown"}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Triggered By</Label>
                    <p className="text-sm text-muted-foreground">{selectedEmergency.triggeredBy || "Unknown"}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Created At</Label>
                    <p className="text-sm text-muted-foreground">
                      {selectedEmergency.createdAt
                        ? format(new Date(selectedEmergency.createdAt), "MMM d, yyyy h:mm a")
                        : "Unknown"}
                    </p>
                  </div>
                  {selectedEmergency.resolvedAt && (
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {format(new Date(selectedEmergency.resolvedAt), "MMM d, yyyy h:mm a")}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  <div className="mt-2 p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center space-x-2 mb-2">
                      <MapPin className="h-4 w-4 text-gray-500" />
                      <span>{selectedEmergency.location?.address || "Unknown location"}</span>
                    </div>
                ...
                {selectedEmergency.description && (
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">{selectedEmergency.description}</p>
                  </div>
                )}
                ...
                {selectedEmergency.responders && selectedEmergency.responders.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2 mt-2">
                      {selectedEmergency.responders.map((responderId, index) => (
                        <div key={index} className="flex items-center space-x-3 p-2 bg-gray-50 rounded-lg">
                          <Avatar>
                            <AvatarFallback>R{index + 1}</AvatarFallback>
                          </Avatar>
                          <div>
                            <div className="font-medium">Responder {index + 1}</div>
                            <div className="text-xs text-muted-foreground">ID: {responderId}</div>
                          </div>
                          {selectedEmergency.assignedResponderId === responderId && (
                            <Badge className="ml-auto bg-blue-100 text-blue-800">Primary</Badge>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                {selectedEmergency.status === "active" && (
                  <Alert className="bg-yellow-50 border-yellow-200">
                    ...
                    <AlertTitle className="text-yellow-800">Active Emergency</AlertTitle>
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <DialogContent className="max-w-md mx-4">
            <DialogHeader>
              ...
              <DialogDescription>Are you sure you want to delete this emergency record?</DialogDescription>
            </DialogHeader>
            <div className="py-4">
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  This action cannot be undone. This will permanently delete the emergency record from the database.
                </AlertDescription>
              </Alert>
            </div>
            <DialogFooter className="flex-col sm:flex-row gap-2">
