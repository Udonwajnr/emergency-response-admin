...
export default function UsersPage() {
  ...
  const calculateStats = (userData) => {
    ...
    const emergencyUnits = userData.filter(
      (u) => u.role === "emergency_unit"
    ).length;
    const admins = userData.filter((u) => u.role === "admin").length;
    const verified = userData.filter(
      (u) => u.isVerified && u.isApprovedByAdmin
    ).length;
    const pending = userData.filter(
      (u) =>
        ...
        (u.documents.nationalID?.status === "pending" ||
          u.documents.certificationFiles?.some(
  ...
  const handleReviewDocument = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setUsers((prev) =>
          prev.map((user) =>
            user._id === documentToReview.userId ? result.data.user : user
          )
        );
    ...
    if (user.role === "client") {
      ...
      if (user.isVerified && user.isApprovedByAdmin) {
        ...
        (user.documents.nationalID?.status === "pending" ||
          user.documents.certificationFiles?.some(
            (file) => file.status === "pending"
          ))
      ) {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      if (
        activeTab === "verified" &&
        (!user.isVerified || !user.isApprovedByAdmin)
      )
        return false;
      if (
        activeTab === "pending" &&
        ...
        return false;
      if (activeTab === "freelancers" && user.role !== "freelancer")
        return false;
      if (activeTab === "emergency_units" && user.role !== "emergency_unit")
        return false;
        ...
        <Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
          <DialogContent className="max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Comprehensive user information and document management
              </DialogDescription>
            </DialogHeader>
            {selectedUser && (
              <div className="space-y-6">
                ...
                <Tabs defaultValue="details" className="w-full">
                  ...
                  <TabsContent value="details" className="space-y-4">
                    ...
                    {selectedUser.servicesOffered &&
                      selectedUser.servicesOffered.length > 0 && (
                        <div>
                    ...
                    {selectedUser.documents ? (
                      <div className="space-y-4">
                        ...
                        {selectedUser.documents.certificationFiles &&
                          selectedUser.documents.certificationFiles.length >
                            0 && (
                            <Card>
                              <CardHeader>
                                <CardTitle className="text-base flex items-center gap-2">
                              ...
                              <CardContent>
                                <div className="space-y-3">
                                  {selectedUser.documents.certificationFiles.map(
                                    (file, index) => (
                                      <div
                                        key={index}
            ...
            <div className="space-y-4">
              ...
              {reviewData.status === "approved" && (
                <Alert>
                  ...
                  <AlertDescription>
                    This will approve the document and mark the user as
                    verified.
                  </AlertDescription>
                </Alert>
              )}
